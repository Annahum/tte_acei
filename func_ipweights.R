...
func_ipweights <- function(input_data) {
  ...
  model_conditional <- paste("intervention ~ ", paste(confounders_baseline, collapse = " + "), sep = "")
  ...
  #Compute estimated stabilized IP weights
  ...
  #Truncate IPW and stabilised IPW to 99th percentile, and keep only weights and id
  output <- input_data %>%
    mutate(ps = pd_intervention) %>% 
    mutate(ipw_trunc = ifelse(ipw >= quantile(ipw, 0.99), quantile(ipw, 0.99),ipw)) %>%
    mutate(s_ipw_trunc = ifelse(s_ipw >= quantile(s_ipw, 0.99), quantile(s_ipw, 0.99), s_ipw)) %>%
    select(id,  ps, ipw, ipw_trunc, s_ipw, s_ipw_trunc)
